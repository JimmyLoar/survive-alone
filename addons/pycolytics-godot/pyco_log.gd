...
func _sync_project_settings() -> void:
	...
	PycoEvent.default_event.api_key = config.get_value("pycolytics", "api_key", _Plugin.DEFAULT_API_KEY)
	url = (
		config.get_value("pycolytics", "server_url", _Plugin.DEFAULT_SERVER_URL) + _url_suffix
	)
