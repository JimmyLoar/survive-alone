...
func _on_ready_main_camera(camera: MainCameraState):
	if not camera.viewport_rect_changed.is_connected(_on_main_camera_viewport_changed):
		camera.viewport_rect_changed.connect(_on_main_camera_viewport_changed)
